...
cmove *Engine::doaimove() {
	int best_score, depth = 4, top = 0;
	MoveNode *dummy_node = new MoveNode;
	MoveNode *bestmoves[100], *bm;

	best_score = next_ply_best_score(dummy_node, depth);

	if (!dummy_node->child)
		return NULL;

	for (MoveNode *cur=dummy_node->child; cur; cur=cur->next){
		if (cur->score==best_score)
			bestmoves[top++] = cur;
		/*debug.debug(cur->move->getMoveTxt());
		debug.debug("(");
		debug.debug(cur->score);
		debug.debug(");");*/
	}
	...
	bm = bestmoves[rand() % top];
	cmove *move = new cmove(bm->move);
	domove(move);
	if (!bm->child)
		isMate = 1;
...
//
int Engine::next_ply_best_score(MoveNode *par, int depth) {
	generate_moves(par);
	depth--;
	...
	for (MoveNode *cur=par->child; cur; cur=cur->next) {
		...
		if (depth==0)
			...
			cur_score = - next_ply_best_score (cur, depth);
...
cmove *Engine::create_move(bitboard f, bitboard t, byte moved, byte promto = 0, byte flags = 0, bitboard mov2 = 0){
	bitboard atkmoves = 0;
			...
			if (m & emptysq)
			{
				...
				if (lsb & rank2)
				{
					...
					if (m & emptysq)
						...
						par->addChild(create_move (lsb, m, pawn, 0, FLAGDOUBLEMOVE));
				}
			}
	...
	if (!kingmoved[moveof]) {
	   ...
	   if (!rookmoved[moveof][0]) {
          if (moveof==white && !(_all&0xE) && (pieces[white][rook] & 0x1)) {
        	  par->addChild(create_move(lsb, 0x4, king, 0, FLAGCASTLEA | KINGMOVED, 0x1 | 0x8));
          } else if (moveof==black && !(_all&0xE00000000000000LLU) && pieces[black][rook] & 0x100000000000000LLU) {
        	  par->addChild(create_move(lsb, 0x400000000000000LLU, king, 0, FLAGCASTLEA | KINGMOVED, 0x100000000000000LLU | 0x800000000000000LLU));
          }
       }
       // castle towards fileh
       if (!rookmoved[moveof][1]) {
          if (moveof==white && !(_all&0x60) && pieces[white][rook] & 0x80) {
        	  par->addChild(create_move(lsb, 0x40, king, 0, FLAGCASTLEH | KINGMOVED, 0x80 | 0x20));
          }else if (moveof==black && !(_all&0x6000000000000000LLU) && pieces[black][rook] & 0x8000000000000000LLU) {
        	  par->addChild(create_move(lsb, 0x4000000000000000LLU, king, 0, FLAGCASTLEH | KINGMOVED, 0x8000000000000000LLU | 0x2000000000000000LLU));
          }
       }
    }
	// loop thru' all moves
...
cmove *Engine::check_piece_move(bitboard f, bitboard t, int promto, int cap) {
	...
	if (f & pieces[moveof][pawn]) {
		...
		if ((f>>8) & emptysq){
			if (t & (f>>8))
				return new cmove(f, t, promto, pawn);
		...
		if (!kingmoved[moveof] && f & start_pieces[moveof][king]) { // if king hasnt moved
		   ...
		   if (!rookmoved[moveof][0] && t & filec) { // if file A rook hasnt moved
	          if (moveof==white && !((all[white]|all[black])&0xE) && (pieces[white][rook] & 0x1)) {
	        	  return new cmove(f,t,0x1|0x8,king,0,FLAGCASTLEA | KINGMOVED);
	          } else if (moveof==black && !((all[white]|all[black])&0xE00000000000000LLU) && pieces[black][rook] & 0x100000000000000LLU) {
	              return new cmove(f,t,0x100000000000000LLU | 0x800000000000000LLU,king,0,FLAGCASTLEA | KINGMOVED);
	          }
	       }
	       // castle towards fileh
	       if (!rookmoved[moveof][1] && t & fileg) {
	          if (moveof==white && !((all[white]|all[black])&0x60) && pieces[white][rook] & 0x80) {
	        	  return new cmove(f,t,0x80 | 0x20,king,0,FLAGCASTLEH | KINGMOVED);
	          }else if (moveof==black && !((all[white]|all[black])&0x6000000000000000LLU) && pieces[black][rook] & 0x8000000000000000LLU) {
	        	  return new cmove(f,t,0x8000000000000000LLU | 0x2000000000000000LLU,king,0,FLAGCASTLEH | KINGMOVED);
	          }
	       }
	    }
	} else {
